...
BEGIN
    ...
    SET OtherExpensesTotal = IFNULL(total_other_expenses, 0),
        TotalExpenses = (
            ...
            FROM (SELECT TotalMaterials, ElectricianWages, CarpenterWages, MasonWages, PainterWages FROM ProjectSummary WHERE ProjectID = OLD.ProjectID) AS tmp
        ),
        TotalProfit = (
            ...
            FROM (SELECT TotalIncome, TotalMaterials, ElectricianWages, CarpenterWages, MasonWages, PainterWages FROM ProjectSummary WHERE ProjectID = OLD.ProjectID) AS tmp2
        )
    WHERE ProjectID = OLD.ProjectID;
END;